const taskAssignedTemplate = ({ taskTitle, projectName, managerName, taskLink, dueDate }) => {
  const dueDateHtml = dueDate ? `<p style="margin:0 0 16px;color:#475569;font-size:14px;"><strong>Due Date:</strong> ${dueDate}</p>` : "";
        ...
        <table width="100%" cellpadding="0" cellspacing="0" style="max-width:520px;background:#ffffff;border:1px solid #e2e8f0;border-radius:8px;overflow:hidden;box-shadow:0 4px 6px -1px rgba(0, 0, 0, 0.05);">
          <tr>
            <td style="padding:40px 32px 20px;text-align:center;">
              <div style="display:inline-block;width:48px;height:48px;line-height:48px;background-color:#eff6ff;color:#2563eb;border-radius:24px;margin-bottom:16px;font-size:24px;">📋</div>
              <h1 style="margin:0;color:#0f172a;font-size:20px;font-weight:600;letter-spacing:-0.5px;">New Task Assigned</h1>
            </td>
          </tr>
          <tr>
            <td style="padding:20px 32px 32px;">
              <p style="margin:0 0 16px;color:#334155;font-size:16px;line-height:1.6;">
                <strong>${managerName}</strong> has assigned a new task to you in the <strong>${projectName}</strong> project.
              </p>
